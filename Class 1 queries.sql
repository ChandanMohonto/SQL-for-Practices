...
ALTER TABLE students ADD COLUMN testing VARCHAR(15);

-- deleting a column from the table
ALTER TABLE students DROP COLUMN test;

ALTER TABLE students RENAME COLUMN student_course TO course;

-- modifying a column name from the table
ALTER TABLE students CHANGE COLUMN test testing INT;
...
SELECT * FROM products GROUP BY product_quantity HAVING product_price >30;
...
-- finding 3rd highest price of the product
SELECT product_name, product_price FROM products WHERE product_price = (SELECT MAX(product_price) FROM products 
...
CASE
	WHEN product_price<=20 THEN 'All people can buy this product.'
	WHEN product_price<=30 THEN 'Mid-class people can buy this product.'
	ELSE 'Upper-class people can buy this product.'
